...
class PrimeroClientes extends React.Component {
  ...
  const { drizzle } = this.props;
    ...
    const counterKey = instance.methods.nClientes.cacheCall();
  ...
  componentDidUpdate(prevProps, prevState, snapshoot) {
      ...
      const numContracts = drizzleState.contracts.Factoria.nClientes[this.state.counterKey];
      ...
      for (let i=0 ; i<nc ; i++) {
          ...
          contractKeys[i] = this.props.drizzle.contracts.Factoria.methods["clientes"].cacheCall(i);
      }
  ...
  render() {
    ...
    const numContracts = Factoria.nClientes[this.state.counterKey];
    ...
    for (let i=0 ; i<nc ; i++) {
      ...
      const addr = Factoria.clientes[this.state.contractKeys[i]];
    ...
    return (
        <div className="irsFactory-data">
            <h1 className="display-3"> Clientes</h1>
            <h2 className="NC"> Nº de Clientes:  {numContracts && numContracts.value } </h2>
            <ListaClientes drizzle={this.props.drizzle}
...
export default PrimeroClientes;
